...
import { Building, Globe, Star, Users, Briefcase, Flag, ArrowLeft, Pencil, Check, X, ArrowUpRight, Calendar, ShieldCheck } from "lucide-react";
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-white rounded-2xl border border-border/60 shadow-sm p-6 flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {company.tagline && <p className="text-sm font-semibold text-foreground/80 mt-1">{company.tagline}</p>}
          <p className="text-muted-foreground text-sm font-medium mt-1">
            Owned by {company.owner.full_name || company.owner.email} &middot;{" "}
            {company.jobLocation?.name ?? company.region?.name ?? "No location set"}
          </p>
          {company.description && (
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 mt-2">
            ...
            {company.industry && (
              <span className="inline-flex items-center gap-1.5 text-xs font-bold text-muted-foreground">
                <Briefcase className="w-3.5 h-3.5" /> {company.industry.name}
              </span>
            )}
            {company.companyType && (
              <span className="inline-flex items-center gap-1.5 text-xs font-bold text-muted-foreground">
                <ShieldCheck className="w-3.5 h-3.5" /> {TYPE_LABELS[company.companyType]}
              </span>
            )}
            {company.companySize && (
              <span className="inline-flex items-center gap-1.5 text-xs font-bold text-muted-foreground">
                <Users className="w-3.5 h-3.5" /> {SIZE_LABELS[company.companySize]}
              </span>
            )}
            {company.foundedYear && (
              <span className="inline-flex items-center gap-1.5 text-xs font-bold text-muted-foreground">
                <Calendar className="w-3.5 h-3.5" /> Founded {company.foundedYear}
              </span>
            )}
          </div>
          {company.benefits.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-3">
              {company.benefits.map((b) => (
                <span key={b} className="px-2.5 py-1 rounded-lg bg-brand-blue/5 text-brand-blue text-[11px] font-bold border border-brand-blue/10">
                  {b}
                </span>
              ))}
            </div>
